...
public class ClassPathScanHandler {
    ...
    /**
     * get ${parentArtifactId} the classes with annotation.
     *
    ...
    /**
     * get ${parentArtifactId} the sub classes with the specific parent class.
     *
    ...
    private void doScanPackageClassesByJar(String basePackage, URL url, final boolean recursive, Set<Class<?>> classes) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                // 判断是否过滤 inner class
                if (this.excludeInner && name.indexOf('s') != -1) {
    ...
    private boolean filterClassFileByCustomization(File file, boolean recursive) {
        ...
        if (excludeInner && filename.indexOf('s') != -1) {
    ...
    private boolean matchInnerClassname(String className, String filterString) {
        String reg = "^" + filterString.replace("*", ".*") + "${symbol_dollar}";
