...
import com.smart.website.demo.commons.design.test.mock.scan.RegexPatternTypeFilter;
...
public class TestMethodRunCmd extends AbstractCommand {
    ...
    private String completingRegex(String p){
        if(!p.endsWith("${symbol_dollar}")){
            p = p + ".*";
        }
        if(!p.startsWith("^")){
            p = ".*" + p;
        }
        p = p.replaceAll("(${symbol_escape}${symbol_escape}.${symbol_escape}${symbol_escape}*)+", ".*");
        return p;
    }
